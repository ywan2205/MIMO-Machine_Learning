...
with tf.Session() as sess:
    ...
    for i in range(100):
        training_batch = zip(range(0, len(train_data1), batch_size),
                             range(batch_size, len(train_data1)+1, batch_size))
        for start, end in training_batch:
            sess.run(train_op, feed_dict={X: train_data1[start:end], Y: train_label1[start:end],
                                          p_keep_conv: 0.8, p_keep_hidden: 0.5})

        test_indices = np.arange(len(test_data1)) # Get A Test Batch
        np.random.shuffle(test_indices)
        test_indices = test_indices[0:test_size]

        print(i, np.mean(np.argmax(test_label1[test_indices], axis=1) ==
                         sess.run(predict_op, feed_dict={X: test_data1[test_indices],
                                                         p_keep_conv: 0.1,
                                                         p_keep_hidden: 0.1})))
